...
async function requestWargamingData (url, fields, parseItem) {
    ...
    async function requestPage (page) {
        ...
        const items = _(response.data.data).map(parseItem).value()
        data.push(...items)
    }
...
module.exports = (app) => {
    ...
    app.get('/api/worldoftanks', async (req, res) => {
        const data = await getWargamingData(WOT_OPTIONS)
        res.send(data)
    })
    app.get('/api/worldofwarships', async (req, res) => {
        const data = await getWargamingData(WOW_OPTIONS)
        res.send(data)
    })
    app.get('/api/apexlegends', async (req, res) => {
        const data = await getApexLegendsData()
        res.send(data)
    })
}
