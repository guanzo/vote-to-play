        ...
        filteredCandidates ({ candidates }, { activeFilters }) {
            return candidates.filter(candidate => {
                return activeFilters.every(({ id, vmodel, options }) => {
                    if (id === 'name') { return candidate.name.toLowerCase().includes(vmodel.toLowerCase()) } else if (id === 'role' && vmodel !== options[0]) { return candidate.Group === vmodel } else if (id === 'role2' && vmodel !== options[0]) { return candidate.SubGroup === vmodel }
                })
            })
        }
    }
