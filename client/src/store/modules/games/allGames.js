...
const modifiedWhitelistMixin = _.merge({}, whitelistMixin,
    {
        ...
        actions: {
            ...
            partition ({ commit, state, getters }) {
                ...
                if (!state.tempWhitelist.length) { partition.tempWhitelist = state.whitelistedNames } else { partition.tempWhitelist = state.tempWhitelist }
...
const allGames = _.merge({}, gameMixin, modifiedWhitelistMixin, {
    ...
    actions: {
        [ACTIONS.GET_CANDIDATES] ({ commit }) {
            const limit = 30
            return axios.get(`https://api.twitch.tv/helix/games/top?limit=${limit}`, {
                headers: {
                    'Client-ID': process.env.VUE_APP_EXTENSION_CLIENT_ID
                }
            })
                .then(res => {
                    const topGames = res.data.data.map(game => {
                        setImage(game)
                        return game
                    })
...
const placeholderArt = 'https://static-cdn.jtvnw.net/ttv-static/404_boxart-72x100.jpg'

// twitch allows you to give custom dimensions
function setImage (game) {
    const imgTemplateUrl = (
        game.box?.template || // kraken
        game.box_art_url || // helix
        placeholderArt
    )

    game.img = imgTemplateUrl
